...
class Stack {
    ...
    public void reverseStack() {
        if (top == 0) { /** stack has only one element */
            return;
        }
        /** hypothesis */
        int lastElement = pop();
        reverseStack();
        /** induction */
        insertIntoReversedStack(lastElement);
        return;
    }

    // inserting into reversed stack
    void insertIntoReversedStack(int element) {
        if (top == -1) {
            push(element);
            return;
        }
        int lastElement = pop();
        insertIntoReversedStack(element);
        push(lastElement);
        return;
    }
